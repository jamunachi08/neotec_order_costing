...
def _assert_saved(doctype, name):
	if not name or str(name).startswith("new-"):
		frappe.throw("Save the {0} first.".format(doctype), title="Document Not Saved")
	if not frappe.db.exists(doctype, name):
		frappe.throw("{0} {1} does not exist.".format(doctype, name), title="Not Found")
...
def preview_material_request_split(material_request):
	...
	_assert_saved("Material Request", material_request)
	settings = get_settings()
