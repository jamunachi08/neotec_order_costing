...
from neotec_order_costing.api.order_cost import resolve_rate
...
def execute(filters=None):
	...
	rows = _compute(rows, selected, filters.get("cost_basis") or "Actual Purchase Cost")
...
def _dn_conditions(filters):
	...
	mapping = {
		...
		"item_code": "sii.item_code = %(item_code)s",
	}
...
# ---------------------------------------------------------------------------
def _compute(rows, selected, basis="Actual Purchase Cost"):
	for r in rows:
		qty = flt(r.get("_qty"))
		revenue = flt(r.get("_revenue"))
		cogs_actual = flt(r.get("cogs_actual"))

		# The pegged rate stamped on the row may be missing or stale, so fall
		# back through the procurement chain rather than to blended valuation.
		rate = flt(r.get("_pegged_rate"))
		if not rate:
			rate, _src = resolve_rate({
				"sales_order": r.get("_sales_order"),
				"item_code": r.get("_item"),
				"qty": qty,
				"cogs_ledger": cogs_actual,
			}, basis)
		cogs_pegged = rate * qty
		...
		# actual purchase cost leads; ledger valuation is the fallback only
		effective = cogs_pegged if basis == "Actual Purchase Cost" else (cogs_actual or cogs_pegged)
		effective = effective or cogs_actual
		r["cogs_effective"] = effective
		r["gross_profit"] = revenue - effective
		r["gross_margin_pct"] = (r["gross_profit"] / revenue * 100) if revenue else 0
		r["unit_cost"] = (effective / qty) if qty else 0
